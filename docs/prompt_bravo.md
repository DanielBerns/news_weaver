...
The architecture consists of five decoupled components: 
1. **Manager** (Scheduler), 
2. **Extractor** (Ingestion), 
3. **Transformer** (Processor), 
4. **Loader** (API & Storage),
5. **Inspector** (Visualization & Report)

## **2. Core Requirements**

* **Language:** Python 3.14
* **Key Libraries:** * **Core:** `SQLAlchemy` (ORM), `Pydantic` (Validation), `python-crontab` (Scheduling), `PyYAML` (Config).
    * **Web/API:** `FastAPI`, `uvicorn`, `httpx` (Async Client),  `Tailwind` (CSS), `Node.js`
...
**Required Settings:**
* **Database:** Connection strings for `control.db` (Control) and `mosaic.db` (Storage).
* **Web Sources:** a list of bash scripts to be executed, with their respective parameters: remote sources (URLs) to be scrapped, allowed mimetypes, scraping frequencies, identifiers for storage.
* **Local Sources:** a list of bash scripts to be executed, with their respective parameters: dirs to be processed, allowed mimetypes, identifiers for storage.
* **API:** Loader host (e.g., `127.0.0.1`), port, and `secret_key` (for component authentication).
* **Storage:** `base_dir` (absolute path), `collection_dir` (absolute path for raw files).
* **Logging:** Log file path, rotation policy, and log level.
* **System:** Absolute paths to the python interpreter and uv manager (for cron jobs).
...
**Functionality:**
1.  **Source Sync:** Queries `control.db` (table: `sources`) for active sources.
2.  **Extractor Scheduling:** Generates a cron job for each source to run different bash scripts (where the execution of `web_extractor.py`, `rss_extractor.py`, `local_extractor.py` are done using the uv package manager, with the appropiate config file as argument) at the specified schedule.
...
The CLI tools (`web_extractor.py`, `rss_extractor.py`, `local_extractor.py`) triggered by Cron to fetch data.

**Control Database (`control.db`):**
* **`sources` table:**
    * `id` (PK), `uri` (Unique - supports `http://`, `https://`, and `file://`), `type` (WEB, RSS, LOCAL), `schedule` (Cron str), `last_scraped_at`.
* **`scraped_files` table:**
...
A `FastAPI` server (`loader.py`) managing the final data warehouse (`mosaic.db`).

**Storage Database (`mosaic.db`):**
...
* **`images`:** * `id` (PK)
    * `source_file_id` (Unique, FK to control.db ref)
    * `url` (Text)
